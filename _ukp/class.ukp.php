...
// #region @ukp db
     ...
     * @version 2026.03.31
     *
     * @param  array $row    키: 컬럼명, 값: 컬럼값인 row 배열
     ...
     * @param  int   $depth   1 이상인경우 개행 후 $depth * 4 만큼 들여쓰기
     * @return array          row 정보
     * - `string [set]`     추가 set문
    ...
    function db_create_row($row_arr = array(), $depth = 1) {
        ...
        foreach ($list as $temp) {
            $return_arr[$temp] .= implode(",{$padding}", $arr[$temp]);
        }
        return $return_arr;
    }
     ...
     * 
     * require  2026.03.18 db_create_where
     * @version 2026.03.18
     * 
    ...
    function db_create_where($where_arr = array(), $or_bool = false, $depth = 1) {
        ...
        foreach ($where_arr as $k => $v) {
            ...
            //in 쿼리(배열)
                ...
                //빈배열인경우 조건문 변경
                if (count($v) == 0) {
                    $return_arr["where"] .= $operator == "in" ? "1 = 0" : "1 = 1";
                    continue;
                }
                $return_arr["where"] .= "{$field} {$operator} (\n{$padding}    ";
                $sep = "";
                foreach ($v as $temp) {
                    $return_arr["where"] .= $sep . "?";
     ...
     * 
     * require  2026.03.31 db_result_array db_row_array db_select_sql
     * @version 2026.03.31
     * 
     * @param  string $table    테이블명
     * @param  array  $option   옵션
     * - `array  [select=array()]`        select 컬럼 리스트, 빈배열인경우 sql파일 select 그대로 사용
     * - `array  [where=array()]`         where문, 키는 컬럼명, 값은 컬럼값
     * - `bool   [or_bool=false]`         true: where or문, false: where and문
     * - `bool   [delete_flag_bool=true]` true - 삭제여부 사용, false - 삭제여부 사용안함, 삭제여부는 y, n 값으로 판단
     * - `bool   [where_table_bool=true]` true 인경우 where 문에 축약테이블명 필수, ex) array("`st`.`field`" => "value")
     ...
     * - where 없는경우 입력만 됨
     * 
     * require  2026.03.31 db_add_row db_add_table_info db_create_row db_create_where db_delete db_query db_result_array db_row_array
     * @version 2026.03.31
     * 
     * @param  string $table    테이블명
     * @param  array  $option   옵션
     * - `array  [list=array()]`     키는 컬럼명, 값은 컬럼값인 배열 리스트
     * - `array  [where=array()]`    수정 조건문, 키는 컬럼명, 값은 컬럼값
     * - `string [primary=""]`       테이블 기본키 컬럼명, 필수값
     * - `bool   [or_bool=false]`    true: where or문, false: where and문
     * - `bool   [force_bool=false]` true: 삭제, false(기본값): delete_flag 변경, delete_flag 없는경우 force_bool true로 변경
     * - `string [prefix=null]`      테이블 접두어, 세팅 안한경우 설정값
     * - `string [delete_flag=null]` 삭제여부컬럼, 세팅 안한경우 설정값
    ...
    function db_update_multiple($table, $option = array(), $database = "default") {
        $main_list = isset($option["list"]) ? $option["list"] : array();
        $main_where = isset($option["where"]) ? $option["where"] : array();
        $main_primary = isset($option["primary"]) ? strtolower($option["primary"]) : "";
        $or_bool = isset($option["or_bool"]) ? $option["or_bool"] : false;
        $force_bool = isset($option["force_bool"]) ? $option["force_bool"] : false;
        $result = $this->db_add_table_info($table, $database);
        $prefix = isset($option["prefix"]) ? $option["prefix"] : $result["prefix"];
        ...
        //mysql 8.0.20 이상만 문법 다르기 때문에 버전체크
        $different_text = "";
        $sql = "select version() as `v`";
        $result = $this->db_row_array($sql);
        do {
            if (stristr($result["v"], "MariaDB")) {
                break;
            }
            preg_match('/^(\d+\.\d+\.\d+)/', $result["v"], $arr);
            $version = isset($arr[1]) ? $arr[1] : $result["v"];
            if (version_compare($version, "8.0.20", ">=")) {
                $different_text = "as `new_data`\n";
            }
        } while (false);
        //select
        $where_info = $this->db_create_where($main_where, $or_bool);
        ...
        $sq .= "    `{$main_primary}`\n";
        $binding = $where_info["binding"];
        $current_list = $this->db_result_array($sq, $binding, $database);
        //입력값 세팅
        $into = "";
        $values = array();
        $binding = array();
        $update = "";
        $i = 0;
        foreach ($main_list as $temp) {
            //컬럼배열 다시 세팅(기본키가 auto_increment 아닌경우 대응)
            $arr = array();
            $primary_key = "";
            $primary_value = "";
            foreach ($temp as $k => $v) {
                $temp_key = explode(" ", trim($k));
                $temp_key = str_replace(array("`", " "), "", strtolower($temp_key[0]));
                //기본키 컬럼이 배열에 있는경우
                if ($temp_key == $main_primary) {
                    $primary_key = $k;
                    $primary_value = $v;
                    continue;
                }
                //삭제여부컬럼은 무시
                if (strval($delete_flag) != "" && $temp_key == $delete_flag) {
                    continue;
                }
                $arr[$k] = $v;
            }
            //기본키 컬럼 추가
            if (isset($current_list[$i])) {
                $arr[$main_primary] = $current_list[$i][$main_primary];
            } else if ($primary_key != "") {
                $arr[$primary_key] = $primary_value;
            } else {
                $arr["{$main_primary} is"] = "null";
            }
            //삭제여부 컬럼 추가
            if (strval($delete_flag) != "") {
                $arr["{$delete_flag} is"] = "'n'";
            }
            $temp_add_row = $this->db_add_row($arr, array(
                "date" => $update_date,
                "time" => $update_time,
                "datetime" => $update_dt
            ));
            $result = $this->db_create_row($temp_add_row, 0);
            //값 세팅
            if ($i == 0) {
                $into = $result["into"];
                $arr = explode(",", $into);
                foreach ($arr as $k => $v) {
                    ...
                    $update .= "{$v} = " . ($different_text == "" ? "values({$v})" : "`new_data`.{$v}");
                }
            } else if ($into != $result["into"]) {
                trigger_error("모든 list의 컬럼 위치 및 갯수가 일치해야합니다.");
                exit;
            }
            $values[] = "({$result["values"]})";
            $binding = array_merge($binding, $result["binding"]);
            $i++;
        }
        //삭제값 세팅
        $delete_in = array();
        foreach ($current_list as $k => $v) {
            ...
            $delete_in[] = $v[$main_primary];
        }
        //컬럼 검증, 리스트 없는경우 검증 안함
        if (count($main_list) > 0) {
            ...
            $sq .= "    `{$prefix}{$table}`\n";
            $sq .= "where\n";
            $sq .= "    `{$main_primary}` is null";
            $this->db_query($sq, array(), $database);
        }
        //삭제
        $option = array(
            "where" => array(
                "{$main_primary} in" => $delete_in
            ),
            "or_bool" => $or_bool,
        ...
        if (count($main_list) > 0) {
            ...
            $sq .= "    {$update}";
            $this->db_query($sq, $binding, $database);
        }
    }
// #endregion
// #region @ukp decode
...
// #region @ukp input
     ...
     * - 배열파일 일괄업로드
     * 
     * require  2026.02.06 input_file unique_id
     * @version 2026.02.06
     *
     * @param  string $name             파일 태그 name속성
     * @param  string $src              파일 저장할 폴더경로
    ...
    /**
     * - base64 문자열로 되어있는 파일을 저장
     * - 함수 실행 후 input_file_upload_info 함수 호출하여 반환값으로 결과확인
     * - 업로드 파일명은 유니크한 파일명
     * 
     * require  2026.03.31 array_value unique_id $input_upload_info
     * @version 2026.03.31
     *
     ...
     * @return bool
     */
    function input_file_upload_base64($file, $src = ".", $allow_ext = array()) {
        //폴더존재확인
        if (!is_dir($src)) {
            $this->input_upload_info["code"] = "3";
            return false;
        ...
        file_put_contents("{$src}/{$file_name}{$temp_num}.{$ext}", $this->decode_base64($file));
        $this->input_upload_info["code"] = "1";
        $this->input_upload_info["name"] = "{$file_name}{$temp_num}";
        $this->input_upload_info["ext"] = "{$ext}";
        $this->input_upload_info["full_name"] = "{$file_name}{$temp_num}.{$ext}";
